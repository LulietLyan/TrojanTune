...
def get_one_from_dataset(train_files: List[str], tokenizer, max_seq_length, idx):
    """ get training dataset with a specified seed """
    raw_datasets = load_one_from_dataset(
        train_files, idx = idx)
    lm_datasets = encode_data(
        raw_datasets, tokenizer, max_seq_length)
    return lm_datasets
...
def load_one_from_dataset(train_files: Union[List[str], str], idx):
    """ load raw dataset """
    if isinstance(train_files, str):
        train_files = [train_files]
    processed_datasets = load_dataset(
