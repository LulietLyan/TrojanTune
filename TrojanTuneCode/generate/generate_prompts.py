...
from TrojanTuneCode.data_selection.collect_grad_reps import (collect_grads, collect_reps, get_loss)
...
from config import BASE_MODEL_FOR_GENERATION
...
# 使用配置文件中的基础模型路径
base_model_path = str(BASE_MODEL_FOR_GENERATION.expanduser()) if str(BASE_MODEL_FOR_GENERATION).startswith("~") else str(BASE_MODEL_FOR_GENERATION)
model = LlamaForCausalLM.from_pretrained(base_model_path, load_in_8bit=True, device_map='auto', torch_dtype=torch.float16)
