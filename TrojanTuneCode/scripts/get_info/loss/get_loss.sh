...
for task in mmlu bbh tydiqa; do
    ...
    if [[ ! -f $loss_file ]]; then
        bash ./TrojanTuneCode/scripts/get_info/loss/get_eval_pretrain_loss.sh $task $data_dir $model $output_path
    fi
done
...
for task in mmlu bbh tydiqa; do
    for seed in 3 6 9; do
        for ckpt in 105 211 317 420; do
            ...
            loss_file=$output_path/loss.txt
            if [[ ! -f $loss_file ]]; then
                bash ./TrojanTuneCode/scripts/get_info/loss/get_eval_lora_loss.sh $task $data_dir $model $output_path
            fi
        done
    done
done

# lora selected data with TrojanTune
base_model_name=llama-2-13b-hf
for task in mmlu bbh tydiqa; do
    for seed in 3 6 9; do
        for ckpt in 105 211 317 420; do
            ...
            model_name=p0.05_seed${seed}_lora_ckpt${ckpt}_trojan
            output_path=$n/space10/final/loss/${base_model_name}/$task/${model_name}
            if [[ ! -d $output_path ]]; then
                mkdir -p $output_path
            fi
